...
4. 基于文本中提供的字幕信息在小标题(大标题请忽略。)上增加时间范围, 放在小标题后面。保留时间区间,格式为[hh:mm:ss-hh:mm:ss]。如果 hh 为 00 则隐藏 hh。
5. 基于文本中提供的字幕信息在合适的位置上插入要截图的记号, 规定为 #image[秒数], 例如 #image[20] 表示截取第 20 秒的内容。必须严格按照 #image[整数秒数]。标记占单独的一行`,
